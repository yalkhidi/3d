...
/*   Updated: 2025/12/04 18:04:40 by yalkhidi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	print_content(t_resources *c)
{
	int	i;
	int	j;
...
int	main(int argc, char *argv[])
{
	...
	init_game(&game);
	calculate(&game);
	mlx_hook(game.screen.win, 2, 0, key_handlers, &game);
	mlx_hook(game.screen.win, 17, 0, close_window, &game);
