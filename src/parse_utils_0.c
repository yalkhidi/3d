...
/*   Updated: 2025/12/01 13:19:07 by yalkhidi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	add_mapline(char *str, t_resources *resources, int done)
{
	...
	if (done || !resources->tex_path[0] || !resources->tex_path[2]
		...
		return (clean_and_print_error("Wrong scene descriptor format\n", NULL, 0));
	i = 0;
	while (str[i] && str[i] != '\n')
	{
...
static int	assign_int(int *val, char *str)
{
	...
	if (ft_isdigit(*str) == 0)
		return (clean_and_print_error("Wrong RGB value\n", NULL, 0));
	*val = 0;
...
int	assign_rgb(int *rgb, char *str)
{
	...
	if (rgb[0] != -1)
		return (clean_and_print_error("Element duplicate\n", NULL, 0));
	i = 0;
	while (i < 3)
	{
	...
	while (ft_isspace(*str))
		str++;
	if (*str && *str != '\n')
		return (clean_and_print_error("Unexpected symbol in RGB value\n", NULL, 0));
	return (1);
}
